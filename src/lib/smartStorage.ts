...
export const smartStorageAdapter: StateStorage = {
  ...
  setItem: async (name: string, value: string): Promise<void> => {
    ...
    const hasChanged = (slice: any, key: string) => {
    ...
    if (JSON.stringify(projectSlice) !== JSON.stringify(pickSlice(lastPersistedState, 'projects'))) {
    ...
    if (JSON.stringify(settingsSlice) !== JSON.stringify(pickSlice(lastPersistedState, 'settings'))) {
