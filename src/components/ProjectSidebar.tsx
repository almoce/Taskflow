  ...
  return (
    ...
    >
      <div className="h-12 px-3 flex items-center border-b border-border">
        <div
          className={`flex items-center overflow-hidden transition-all duration-300 ${collapsed ? "w-6" : "w-32"}`}
        >
          <img
      ...
      <ScrollArea className="flex-1 px-2">
        <div className="space-y-0.5">
          {projects.map((project) => {
            const isSelected = activeView === "tasks" && selectedProjectId === project.id;
            return (
              <div
                key={project.id}
              ...
              >
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <div className={`flex items-center ${collapsed ? "" : "gap-2 w-full"}`}>
                        {project.icon ? (
                  ...
                  {collapsed && (
                    <TooltipContent
                      side="right"
                      className="origin-left data-[state=delayed-open]:animate-tooltip-expand data-[state=closed]:animate-tooltip-collapse animation-duration-500"
                    >
                ...
                {!collapsed && (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-40">
                      <DropdownMenuItem onClick={() => onEditProject(project)} className="text-sm">
                        <Pencil className="h-3.5 w-3.5 mr-2" />
                        Edit
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => onExport(project.id)} className="text-sm">
      ...
      <div className="p-2 border-t border-border">
        <Button
          variant="ghost"
          size="sm"
          className={`w-full h-8 px-2 text-muted-foreground hover:text-foreground ${collapsed ? "justify-center" : "justify-start"}`}
