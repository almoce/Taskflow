        ...
        <Card className="bg-zinc-950/40 backdrop-blur-md border border-white/10 overflow-hidden relative group shadow-xl">
          ...
          <CardContent className="p-4 relative">
            <div className="grid grid-cols-7 gap-1 text-center mb-2">
              {["S", "M", "T", "W", "T", "F", "S"].map((d) => (
                <div key={d} className="text-[8px] text-zinc-600 font-medium">
                  {d}
                </div>
              ))}
            </div>
            <div className="grid grid-cols-7 gap-1 text-center">
              {Array.from({ length: 31 }, (_, i) => i + 1).map((day) => (
                <div
                  key={day}
                  className={`text-[10px] py-1 rounded-sm relative ${day === 24 ? "bg-purple-500 text-white font-bold" : "text-zinc-400"}`}
                >
                  {day}
