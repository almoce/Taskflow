...
import { Task, Project, Priority, TaskStatus, TaskTag } from '@/types/task';
...
export function TaskSearch({ tasks, projects, onUpdateTask, onDeleteTask, onSelectProject }: TaskSearchProps) {
  ...
  const hasActiveFilters = searchQuery || selectedPriorities.length > 0 || selectedStatuses.length > 0 || selectedTags.length > 0 || selectedProjects.length > 0;
  const activeFilterCount = selectedPriorities.length + selectedStatuses.length + selectedTags.length + selectedProjects.length;
                ...
                filteredTasks.map(task => {
                  ...
                  return (
                    <div key={task.id} className="relative">
                      <div
                        className="absolute -left-2 top-3 w-1 h-6 rounded-full cursor-pointer hover:scale-110 transition-transform"
                        style={{ backgroundColor: project?.color }}
