  ...
  return (
    ...
    >
      {isNumber ? (
        ...
        <span className="flex items-center justify-center text-foreground/80 pb-[0.1em]">{value}</span>
      )}
    </div>
  );
}
...
export function FocusTimerView({
  ...
  return (
    <div className="relative w-full h-full flex flex-col animate-in fade-in zoom-in duration-500" onClick={() => setActiveFocus(null)}>
      ...
      <div className="absolute top-0 right-0 z-20 flex flex-col items-center gap-2">
        <TooltipProvider>
          ...
          <Popover open={showConfirmResume} onOpenChange={(open) => {
              ...
              <PopoverContent side="left" sideOffset={10} className="w-auto p-2 z-[150] bg-background/95 backdrop-blur-sm border-border/50 shadow-xl">
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className="relative group flex items-center justify-center text-[140px] leading-none font-bold tracking-tighter tabular-nums font-sans">
            ...
            <EditableGroup 
                ...
                onActiveIndexChange={(i) => setActiveFocus(i !== null ? { unit: 'hours', index: i } : null)}
                onExitNext={() => setActiveFocus({ unit: 'minutes', index: 0 })}
            />
            
            <div className="relative h-[1em] flex justify-center overflow-hidden w-[0.2em]">
                <span className="flex items-center justify-center text-foreground/80 pb-[0.1em]">:</span>
            </div>
            ...
            <EditableGroup 
                ...
                onActiveIndexChange={(i) => setActiveFocus(i !== null ? { unit: 'minutes', index: i } : null)}
            ...
            <EditableGroup 
                ...
                onActiveIndexChange={(i) => setActiveFocus(i !== null ? { unit: 'seconds', index: i } : null)}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-center w-full z-20">
        <p className="text-[10px] text-muted-foreground/40 font-medium uppercase tracking-widest">
          ...
          {isRunning ? " pause" : " resume"} • <kbd className="px-1.5 py-0.5 rounded border border-border/50 bg-background/20">Esc</kbd> to {isRunning ? "end" : "discard"}
        </p>
      </div>
    </div>
  );
