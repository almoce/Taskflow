...
describe("smartStorageAdapter", () => {
  ...
  describe("getItem", () => {
    ...
    it("should merge granular keys into a single state object", async () => {
      (get as any).mockImplementation((key: string) => {
        if (key === STORAGE_KEYS.PROJECTS) return JSON.stringify({ state: { projects: [{ id: "p1" }] }, version: 0 });
        if (key === STORAGE_KEYS.TASKS) return JSON.stringify({ state: { tasks: [{ id: "t1" }] }, version: 0 });
        if (key === STORAGE_KEYS.ARCHIVE) return JSON.stringify({ state: { archivedTasks: [] }, version: 0 });
        if (key === STORAGE_KEYS.AUTH) return JSON.stringify({ state: { session: { user: "u1" } }, version: 0 });
        if (key === STORAGE_KEYS.SETTINGS) return JSON.stringify({ state: { activeView: "calendar" }, version: 0 });
        return undefined;
      });
    ...
    it("should migrate legacy data if granular keys are missing but legacy key exists", async () => {
        ...
        (get as any).mockImplementation((key: string) => {
            if (key === STORAGE_KEYS.LEGACY) return JSON.stringify({ state: { projects: [{id: "legacy"}] }, version: 0 });
            return undefined;
        });
  ...
  describe("setItem", () => {
    it("should split state and write to granular keys", async () => {
      ...
      expect(set).toHaveBeenCalledWith(STORAGE_KEYS.PROJECTS, expect.stringContaining('"projects":[{"id":"p1"}]'));
      expect(set).toHaveBeenCalledWith(STORAGE_KEYS.TASKS, expect.stringContaining('"tasks":[{"id":"t1"}]'));
    });
